...
imgname  = "9recto3_proc-Exemple.jpg"                                                   # The name of the image we are analyzing
Workfold = Path('/media/pa/Storage/Paul_VALCKE_THESE_FULL/0--IMAGES/QUALITYSQUEL/DONE') # The folder the image is stocked
SavedBase ='Results/Gorgones/'                                                          # Where the result will be placed
...
for _ in range(1):
    ...
    p={}
    ### PARAMETRES IMAGE
    p['resize']=1            # Réduit la dimension de l'image si nécessaire. ça va beaucoup plus vite ainsi, mais on perd en précision. >1 pour réduire
    ...
    for _ in range(1):
        Savefold = Path(SavedBase+imgname.split('.')[0])
...
### PREPARATION DES DONNEES INITIALES ###
try :
    print('Try to reload old datas...')
    ''' Try to get the datas of previous runs'''
...
for _ in range(1):### LINK ###
    ...
    for index,P in enumerate(                [P for P in AllPlaces.list if (not len(P.Links) and len(P.Extremities)>1 and not P.ThereisnoLink)]) :
        ...
        for i,E in enumerate(P.Extremities):
            ...
            Nimg     = cv2.circle(Nimg   ,(int(pt[0]),int(pt[1])),2*voiesWidth,(127+30*i  ,127-10*i,127+10*i),-1)
